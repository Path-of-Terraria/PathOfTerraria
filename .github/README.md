...
3) We use [Subworld Library](https://github.com/jjohnsnaill/SubworldLibrary) for subworlds.
   - It is an ordinary Workshop dependency. The namespace is `SubworldLibrary`.
   - `lib/SubworldLibrary.dll` is a compile-time reference only; tModLoader resolves the real mod
     at load time from `modReferences` in `build.txt`.
   - Extract `SubworldLibrary.dll` from the installed `SubworldLibrary.tmod` to refresh the
     reference after a library update.
   - We previously maintained a fork, `SubworldLibraryCommunityFork`, carrying multiplayer
     traversal and player-visibility fixes. Upstream has since addressed those, so the fork was
     retired.
