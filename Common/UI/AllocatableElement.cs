...
{
	private const float SearchPulseSpeed = 0.10f;
	private const float SearchPulseAmplitude = 0.15f;
	private const float SearchPulseBaseline = 0.7f;
	...
	private void DrawSearchHighlight(SpriteBatch spriteBatch, Vector2 center, float scale)
	{
		...
		float nodeScale = Math.Max(Node.Size.X, Node.Size.Y) / glow.Width * 3.8f * scale;
