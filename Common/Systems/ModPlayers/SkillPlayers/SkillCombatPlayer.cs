...
{
	private const int CombatModeSlot = 0;
	private const int BuildingModeSlot = 1;
	private const int LastHotbarSlot = 9;
	...
	public override void SetControls()
	{
		...
		HandleModeLockedMouseWheel();

		if (!NewHotbar.InCombatMode(Player))
		{
			return;
		}

		for (int i = BuildingModeSlot + 1; i <= LastHotbarSlot; ++i)
		{
			PlayerInput.Triggers.Current.KeyStatus["Hotbar" + (i + 1)] = false;
		}
	}

	private void HandleModeLockedMouseWheel()
	{
		if (PlayerInput.ScrollWheelDelta == 0 || PlayerInput.IgnoreMouseInterface)
		{
			return;
		}

		PlayerInput.LockVanillaMouseScroll($"{PoTMod.ModName}/HotbarModeSeparation");

		if (Player.selectedItem == CombatModeSlot)
		{
			return;
		}

		int offset = PlayerInput.ScrollWheelDelta < 0 ? 1 : -1;
		int nextSlot = Player.selectedItem + offset;

		if (nextSlot < BuildingModeSlot)
		{
			nextSlot = LastHotbarSlot;
		}
		else if (nextSlot > LastHotbarSlot)
		{
			nextSlot = BuildingModeSlot;
		}
	...
	public override void ProcessTriggers(TriggersSet triggersSet)
	{
		...
		if (IsSkillKeyJustPressed(Skill1Keybind))
		{
			UseSkill(0);
		}

		if (IsSkillKeyJustPressed(Skill2Keybind))
		{
			UseSkill(1);
		}

		if (IsSkillKeyJustPressed(Skill3Keybind))
		{
			UseSkill(2);
		}
	}

	private static bool IsSkillKeyJustPressed(ModKeybind keybind)
	{
	...
	private static bool TryParseKeyboardKey(string assignedKey, out Keys key)
	{
		...
		{
			return Enum.TryParse("D" + assignedKey, out key);
		}

		return Enum.TryParse(assignedKey, out key);
	}
