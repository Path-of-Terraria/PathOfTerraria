...
internal sealed class ScarabMapLayer : ModMapLayer
{
	...
	public override void Load()
	{
		...
		nemesisIcon = ModContent.Request<Texture2D>($"Terraria/Images/Item_{ItemID.ScarabBomb}");
	...
	{
		if (!ScarabSystem.IsExplorationMap()) { return; }

		foreach (NPC npc in Main.ActiveNPCs)
		{
			Asset<Texture2D> icon;
			string label;
			if (npc.GetGlobalNPC<ScarabEncounterNPC>().Nemesis)
			{
				icon = nemesisIcon;
