...
public abstract class PoTItem : ModItem
{
	...
	internal static readonly List<Tuple<float, ItemRarity, int>> AllItems = [];
	...
	/// <returns></returns>
	public static Color GetRarityColor(ItemRarity rare)
	{
		return rare switch
		{
	...
	/// <returns></returns>
	public static string GetDescriptor(ItemType type, ItemRarity rare, Influence influence)
	{
	...
	public static void GenerateItemList()
	{
		...
		foreach (Type type in PathOfTerraria.Instance.Code.GetTypes())
		{
			...
			int id = PathOfTerraria.Instance.Find<ModItem>(instance.Name).Type;

			if (instance.IsUnique)
			{
				AllItems.Add(new(instance.DropChance, ItemRarity.Unique, id));
			}
			else
			{
				...
				{
					AllItems.Add(new(instance.DropChance * 0.70f, ItemRarity.Normal, id));
				}

				AllItems.Add(new(instance.DropChance * 0.25f, ItemRarity.Magic, id));
				AllItems.Add(new(instance.DropChance * 0.05f, ItemRarity.Rare, id));
			}
		}

		foreach ((float dropChance, int itemType) in ManuallyLoadedItems) 
		{
			AllItems.Add(new(dropChance * 0.7f, ItemRarity.Normal, itemType));
			AllItems.Add(new(dropChance * 0.25f, ItemRarity.Magic, itemType));
			AllItems.Add(new(dropChance * 0.5f, ItemRarity.Rare, itemType));
		}
	...
	public virtual string GenerateName()
	{
		return Rarity switch
		{
			...
			ItemRarity.Magic => $"{GeneratePrefix()} {Item.Name}",
			ItemRarity.Rare => $"{GeneratePrefix()} {Item.Name} {GenerateSuffix()}",
