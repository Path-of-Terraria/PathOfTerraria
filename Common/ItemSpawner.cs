...
internal class ItemSpawner
{
	...
	public static int SpawnRandomItem(Vector2 pos, Func<Tuple<float, ItemRarity, int>, bool> dropCondition,
	...
	private static int SpawnItemFromList(Vector2 pos, Func<Tuple<float, ItemRarity, int>, bool> dropCondition, int ilevel,
		float dropRarityModifier, List<Tuple<float, ItemRarity, int>> filteredGear)
	{
		dropRarityModifier += ilevel / 10f; // the effect of item level on "magic find"

		// Calculate dropChanceSum based on filtered gear
		float dropChanceSum = filteredGear.Where(x => dropCondition(x)).Sum((Tuple<float, ItemRarity, int> x) =>
			PoTItem.ApplyRarityModifier(x.Item1, dropRarityModifier));
		float choice = Main.rand.NextFloat(dropChanceSum);

		float cumulativeChance = 0;

		foreach (Tuple<float, ItemRarity, int> item in filteredGear)
		{
	...
	public static int SpawnItem<T>(Vector2 pos, int itemLevel = 0, ItemRarity rarity = ItemRarity.Normal) where T : PoTItem
	{
	...
	/// <param name="itemLevel">The item level of the item to spawn</param>
	/// <param name="dropRarityModifier">Rolls an item with a drop rarity modifier</param>
	public static int SpawnItemFromCategory<T>(Vector2 pos, int itemLevel = 0, ItemRarity rarity = ItemRarity.Normal) where T : PoTItem
	{
